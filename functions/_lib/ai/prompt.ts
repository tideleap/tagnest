...
export function buildCategorizePrompt(
  ...
  const lines: string[] = [
    [
      ...
      '- 归属路径以【两级为主】：一级是稳定、宽泛、可长期复用的【领域】（如「开发技术」「云服务」「人工智能」）；二级是该书签所属的具体【网站 / 产品 / 主题】（如「高德地图」「React」「阿里云 OSS」）——二级要做到「看到名字就知道是哪个站点」。仅在二级之下确实还需细分（同一产品有明显不同的子模块）时才加第三级，且第三级同样要具体。',
      '- 第三级（子模块）的硬性触发条件，必须【同时满足】：① 同一二级下出现 ≥2 个明显不同的子模块；② 每个子模块自身会被 ≥2 条书签复用；③ 子模块名具体可辨（如「React › Router」「阿里云 OSS › 控制台」）。不满足条件就不要加第三级。',
      '- 一级分类必须复用「已有分类树」中已有的领域；除非都不合适才新建，且新建一级必须是宽泛、长期可复用的领域，不要新建与已有领域重叠或近义的一级。',
      '- 不要为凑层级硬造空泛的中间层，也不要停在只有一个领域的过粗粒度：绝大多数书签应归到「领域 > 网站」两级。内容只够判断一级时，后面几段填 null。',
    ...
    [
      ...
      '字段说明：',
      '- i: 书签序号，从 1 开始，与输入顺序一致。',
      '- path: 归属路径数组，从一级到三级，最具体的一层放最后。推荐两级：["领域","具体网站/产品"]（如 ["开发技术","React"]）；只有一级把握时填 ["一级分类"]；确有必要才填三级 ["领域","网站","子模块"]。至少一段，最多三段。',
