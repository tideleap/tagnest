...
export function buildCategorizePrompt(
  ...
  const lines: string[] = [
    [
      ...
      '- 归属路径以【两级为主】：一级是稳定、宽泛、可长期复用的【领域】（如「开发技术」「云服务」「人工智能」）；二级是该书签所属的具体【网站 / 产品 / 主题】（如「高德控制台」「React 官网」「阿里云 OSS」）——二级要做到「看到名字就知道是哪个站点」。仅在二级之下确实还需细分（如同一产品有明显不同的子模块）时才加第三级，且第三级同样要具体。',
    ...
    [
      ...
      '字段说明：',
      '- i: 书签序号，从 1 开始，与输入顺序一致。',
      '- path: 归属路径数组，从一级到三级，最具体的一层放最后。推荐两级：["领域","具体网站/产品"]（如 ["开发技术","React 官网"]）；只有一级把握时填 ["一级分类"]；确有必要才填三级 ["领域","网站","子模块"]。至少一段，最多三段。',
