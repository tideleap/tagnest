...
> 登记 48 条（其中 2 条为跨文档别名），独立需求 46 项：✅ done 37 ／ ➖ superseded 2 ／ ⬜ open 6 ／ ⏸ blocked-external 1
...
| `F-P1-6` | AI 能力：接线或明确下线 | P1 | ⬜ open | → O11 | 配置层与加密已就绪但推理未接；执行单元见 O11。 |
...
| `O11` | AI 自动标签 / 摘要 | P1 | ⬜ open | 0/2 | 未配置 provider key 时静默降级为 no-op，不阻塞保存主流程。 |
