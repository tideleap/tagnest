...
- 智能集合（B-8）：在图书馆一键把当前筛选（关键词 / 标签 / 作用域 / 排序）「保存为智能集合」；成员由 `listBookmarks` 实时计算，新匹配的书签即时出现在集合内，无需手动维护。智能集合不可手动增删成员（改用手动集合），详情页展示「实时」徽章与查询摘要。设计见 [`docs/SMART-COLLECTIONS-B8-2026-08-16.md`](docs/SMART-COLLECTIONS-B8-2026-08-16.md)。
- 相似书签推荐（B-9）：打开任一书签，编辑器内展示按相似度排序的「相关书签」面板，点击即跳转到该书签的编辑视图。相似度采用多信号启发式——标签 Jaccard（`0.60`）+ 同域名（`0.25`）+ 标题/描述/笔记文本 Dice（`0.15`）——在候选池内实时打分、确定性可测，契合单用户离线优先、零知识私密的产品定位（不引入向量库 / AI 嵌入）。私密书签不进入候选池，私密源自身也只基于非私密库计算，隐私边界不泄漏。设计见 [`docs/SIMILAR-BOOKMARKS-B9-2026-08-16.md`](docs/SIMILAR-BOOKMARKS-B9-2026-08-16.md)。
- 嵌套标签（B-10）：标签支持层级——在标签页建/改标签时可选「父级」，自动排除自身及其子树以防环；侧栏与标签页点击父标签即按整棵子树过滤书签（复用既有 `?tagIds` OR 过滤，后端零改动）。环检测与私密级联在服务端 `validateTagParent` / `setTagPrivate` 已就位，私密子树不跨隐私边界泄漏。设计见 [`docs/NESTED-TAGS-B10-2026-08-16.md`](docs/NESTED-TAGS-B10-2026-08-16.md)。
...
- 统计报告页（A3）：侧边栏「报告」一页看全库画像——规模/收藏/待整理/健康分头条、180 天收藏趋势条形图、标签分布 Top 15、AI 价值加权贡献与采纳率、重复/孤儿标签汇总；纯组合现有统计端点，数据口径与各处 metrics 完全一致。
