  ...
  return (
    <div className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden px-6 text-center">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-1/3 h-96 w-96 -translate-x-1/2 rounded-full bg-brand-wash blur-[100px]" />
      </div>
      ...
      <div className="relative">
        ...
        <p className="mt-6 inline-flex items-center gap-2 text-2xs text-ink-muted">
      ...
      <div className="absolute bottom-0 left-0 right-0 py-6">
        <KineticText duration={32} separator={<span className="text-brand-accent">/</span>}>
          {WORDS.map((w) => (
            <span key={w} className="text-sm font-medium tracking-wide text-ink-muted">
              {w}
            </span>
          ))}
        </KineticText>
